...
def findRelativeRanks(score: List[int]) -> List[str]:
    ...
    for rank, scores in enumerate(sorted_score):
        if "Gold Medal" not in hash_map.values():
            hash_map[scores] = "Gold Medal"
        elif "Silver Medal" not in hash_map.values():
            hash_map[scores] = "Silver Medal"
        elif "Bronze Medal" not in hash_map.values():
            hash_map[scores] = "Bronze Medal"
        else:
            hash_map[scores] = str(rank + 1)

    return [hash_map[i] for i in score if i in hash_map]
