...
bobSizes where aliceSizes[i] is the number of candies of the ith box of candy that Alice has and bobSizes[j] is the
...
have the same total amount of candy. The total amount of candy a person has is the sum of the number of candies in each box they have.

Return an integer array answer where answer[0] is the number of candies in the box that Alice must exchange,
and answer[1] is the number of candies in the box that Bob must exchange. If there are multiple answers, you may return
...
def fairCandySwap(aliceSizes: List[int], bobSizes: List[int]) -> List[int]:
    exchange = (sum(bobSizes) - sum(aliceSizes)) / 2
    bob_uniq_candies = set(bobSizes)

    for candy in aliceSizes:
        if candy + exchange in bob_uniq_candies:
            return [candy, candy + exchange]
...
assert fairCandySwap(aliceSizes=[2, 5], bobSizes=[1, 4]) == [2, 1]
