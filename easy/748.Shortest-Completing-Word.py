...
Hint 1
Count only the letters (possibly converted to lowercase) of each word. If a word is shorter and the count of each
letter is at least the count of that letter in the licensePlate, it is the best answer we've seen yet.
...
def shortestCompletingWord(licensePlate: str, words: List[str]) -> str:
    lp = {i: licensePlate.lower().count(i) for i in licensePlate.lower() if i.isalpha()}
    result = []
    for word in words:
        for letter in lp:
